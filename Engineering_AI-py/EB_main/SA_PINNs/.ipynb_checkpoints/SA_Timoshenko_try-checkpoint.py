...
class NeuralNet(nn.Module):
    ...
    def __init__(self, input_dimension, output_dimension, n_hidden_layers, neurons):
        ...
        self.my_module = nn.Module()
        self.my_module.weight_ic = nn.Parameter(torch.randn(500) + 10, requires_grad=True)
        self.my_module.weight_pde = nn.Parameter(torch.randn(2000) + 10, requires_grad=True)
...
if opt_type == "ADAM":
    optimizer_ = optim.Adam(
        params=[p for p in my_network.parameters() if p.requires_grad and p not in [my_network.my_module.parameters]],
        lr=0.01
    )
    optimizer_col = optim.Adam(my_network.my_module.parameters(), lr=0.001)
elif opt_type == "LBFGS":
    optimizer_ = optim.LBFGS(params=[p for p in my_network.parameters() if p.requires_grad and p not in [my_network.my_module.parameters]],
                             lr=0.1, max_iter=1, max_eval=50000, tolerance_change=1.0 * np.finfo(float).eps)
    optimizer_col = optim.LBFGS(my_network.my_module.parameters(), lr=0.1, max_iter=1, max_eval=50000, tolerance_change=1.0 * np.finfo(float).eps)
else:
    raise ValueError("Optimizer not recognized")


def fit(model, training_set, interior, num_epochs, optimizer, optimizer_col, p, verbose=True):
    history = list()
    ...
    for epoch in range(num_epochs):
        ...
        for j, (initial, u_initial, u_initial_t, bd_left, bd_right, ubl, ubr) in enumerate(training_set):
            def closure():
                ...
                loss_ic = torch.mean((my_network.my_module.weight_ic)**2*(u_initial_pred_.reshape(-1, ) - u_initial.reshape(-1, )) ** p) + \
                          torch.mean((u_init_t.reshape(-1, )) ** p)
                loss_pde = torch.mean((my_network.my_module.weight_pde)**2*(u_tt.reshape(-1, ) + u_xxxx.reshape(-1, ) - (2-pi**2)*torch.sin(interior[:,0])*torch.cos(pi*interior[:,1])) ** p)
                ...
                grad_ic = my_network.my_module.weight_ic.grad
                my_network.my_module.weight_ic.data += 0.0001 * grad_ic

                grad_pde = my_network.my_module.weight_pde.grad
                my_network.my_module.weight_pde.data += 0.0001 * grad_pde
...
# history = fit(my_network, training_set, interior, n_epochs, optimizer_,  optimizer_col, p=2, verbose=True )
# end_time = time.time()
...
# with open('PINN_SA_EB.pkl', 'wb') as f:
...
# torch.save(model_state_dict, 'PINN_SA_EB.pth')
...
with open('PINN_SA_EB.pkl', 'rb') as f:
...
model_state_dict = torch.load('PINN_SA_EB.pth', map_location=torch.device('cpu'))
...
# #plt.savefig('Exact_EB.pdf', dpi = 500, bbox_inches = "tight")
...
# plt.yticks(fontsize=20, fontproperties=custom_font)
# #plt.savefig('Causal_EB.pdf', dpi = 300, bbox_inches = "tight")
# plt.savefig('SA_EB.pdf', dpi = 500, bbox_inches = "tight")
# #plt.savefig('Absolute_error_EB.pdf', dpi = 300, bbox_inches = "tight")
...
plt.savefig('SA_EB.pdf', dpi=500, bbox_inches="tight", format='pdf', backend='cairo')
