...
# Compute the relative L2 error norm (generalization error)
relative_error_test = torch.mean((u_test_pred - u_test) ** 2) / torch.mean(u_test ** 2)
print("Relative Error Test: {:.2f}%".format(relative_error_test.detach().numpy() * 100))

# Create a dark style for the plots
plt.style.use('dark_background')

# Create the plot for Ground Truth
plt.figure(figsize=(10, 6))
plt.grid(True, which="both", ls=":")
plt.plot(x_test, u_test, label="Ground Truth", color='cyan', lw=2, linestyle='--', alpha=0.8)

# Create the plot for Network Prediction
plt.plot(x_test, u_test_pred.detach(), label="Network Prediction", color='magenta', lw=2)
