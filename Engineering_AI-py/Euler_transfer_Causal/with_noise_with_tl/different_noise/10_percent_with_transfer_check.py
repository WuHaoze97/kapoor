...
model_state_dict = torch.load('p3.pth', map_location=torch.device('cpu'))
...
# Calculate relative error and set up the plot
relative_error_test = torch.mean((u_test_pred - u_test) ** 2) / torch.mean(u_test ** 2)
print("Relative Error Test: ", relative_error_test.detach().numpy() * 100, "%")
...
# Set the y-axis ticks to be only [-1, -0.5, 0, 0.5, 1]
plt.yticks([-1, -0.5, 0, 0.5, 1], fontsize=20, fontproperties=ticks_font)

# Scatter the ground truth data and plot the predictions
...
plt.savefig('10_noisy_dotted.pdf', dpi=500, bbox_inches="tight", format='pdf', backend='cairo')
plt.show()
