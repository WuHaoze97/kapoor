...
class NeuralNet(nn.Module):
    ...
    def __init__(self, input_dimension, output_dimension, n_hidden_layers, neurons):
        ...
        self.my_module.weight_pde = nn.Parameter(torch.randn(10000) + 10, requires_grad=True)
    ...
    for epoch in range(num_epochs):
        ...
        for j, (initial, w_initial, w_initial_t, bd_left, bd_right) in enumerate(training_set):
            def closure():
                ...
                # Item 1. below
                loss1 = torch.mean((my_network.my_module.weight_pde)**2*loss_pde) + torch.mean(
                    (my_network.my_module.weight_ic)**2*(u_initial_t.reshape(-1, ) - w_initial_t[:, 0].reshape(-1, )) ** p) + torch.mean(
                ...
                loss2 = torch.mean((my_network.my_module.weight_pde)**2*loss_pde1) + torch.mean(
...
# with open('PINN_tim_SA.pkl', 'wb') as f:
...
# torch.save(model_state_dict, 'SA_PINN_tim.pth')

# +
# loading model

# Load the history from the pickle file
# with open('PINN_tim_SA', 'rb') as f:
...
model_state_dict = torch.load('SA_PINN_tim.pth', map_location=torch.device('cpu'))
...
plt.savefig('SA_PINN_timo_disp.pdf', dpi=500, bbox_inches="tight", format='pdf', backend='cairo')
...
plt.savefig('SA_PINN_timo_rotation.pdf', dpi=500, bbox_inches="tight", format='pdf',  backend='cairo')
#plt.savefig('Causal_EB.pdf', dpi=500, bbox_inches="tight", format='pdf', backend='cairo')
