    ...
    for epoch in range(num_epochs):
        ...
        for j, (initial, u_initial, u_initial_t, bd_left, bd_right, ubl, ubr) in enumerate(training_set):
            def closure():
                ...
                loss = loss_ic + loss_pde + loss_left_b + loss_right_b + torch.mean((loss_e_t)**p) + torch.mean((loss_e_x)**p)
...
with open('PINN_EB_gradient.pkl', 'rb') as f:
...
model_state_dict = torch.load('PINN_EB_gradient.pth',  map_location=torch.device('cpu'))
...
plt.savefig('PINN_EB_gradient.pdf', dpi = 300, bbox_inches = "tight")
...
plt.savefig('PINN_EB_gradient_enhanced.pdf', dpi=500, bbox_inches="tight", format='pdf', backend='cairo')
#plt.savefig('Absolute_error_EB.pdf', dpi = 300, bbox_inches = "tight")
